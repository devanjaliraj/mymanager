...
import { EmpNoteAddAction, EmpNoteEditAction, EmpNoteDeleteAction } from '../store/actions';
...
const NoteModal = ({ row, toggle, isOpen, notes, dispatch, setDeleteModal }) => {
  ...
  const handleNoteSave = (e) => {
    ...
    if (notedata.followUpType != '' && notedata.response != '' && notedata.note != '') {
      dispatch(EmpNoteAddAction(notedata));
  ...
  const handleNoteUpdate = (e) => {
    e.preventDefault();
    dispatch(EmpNoteEditAction(editModalData));
    toggle2();
  ...
  const handleDeleteModal = async (data) => {
    ...
    if (res.value) {
      dispatch(EmpNoteDeleteAction(data?._id));
