...
import { addFinanceCategoryAction, TotalCategoriesAction } from '../store/actions';
...
const LabelManagementModal = (props) => {
  ...
  const handleLabelSubmit = () => {
    ...
    const param = {
      ...
      title: lbTitle,
      labelColor: convertData[0].hex
    };
    // console.log('save label',(colorData.filter((x) => x.hex === lbColor))[0].color);
    selectedLabel._id === '' ? dispatch(addFinanceCategoryAction(param)) : dispatch();
    setSelectedLabel({ _id: '', title: '', color: '' });
    setLbTitle('');
    setLbColor('');
    setModalFlag(false);
