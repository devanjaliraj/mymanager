...
export const AutomationIcon = (props) => {
  ...
  const { item, index, showSide } = props;
  ...
  return (
    <div style={props.rotationIconStyle}>
      <div className="w-100 p-1 text-center" onClick={handleClick('left')}>
        <strong>
        {item.type == 'email' && (
          <img src={sendEmailSvg} alt="email" style={{ marginTop: '7px' }} height="45" width="45" />
        )}
        {item.type == 'text' && (
          <img src={sendTextSvg} alt="email" style={{ marginTop: '7px' }} height="45" width="45" />
        )}
      ...
      <div className="w-100 text-center" style={{ backgroundColor: 'transparent' }}>
        ...
        <br></br>
        <label style={{ fontSize: '11px', color: '#f28422' }}>
          {index === 0 ? (
            <> Immediately</>
          ) : item?.startDate ? (
            <>{item.startDate}</>
          ) : (
            <>{moment(item.sent_date).format('MM/DD/YYYY')}</>
          )}
        </label>
      </div>
      {/* <UncontrolledPopover
