...
const ExpandedComponent = ({ data, store }) => {
  ...
  const handleEdit=(e)=>
  {
    ...
    payload.append('vision',componentData?.vision)
    payload.append('purpose',componentData?.purpose)
    payload.append('obstacle',componentData?.obstacle)
    payload.append('resource',componentData?.resource)
          ...
          <Col md="3" className="mt-auto mb-0">
            <div>
              ...
              <div>
                ...
                <img
                  src={componentData.file===undefined?data?.pictureUrl: URL.createObjectURL(componentData?.file)}
...
const TaskList = (props) => {
 ...
 useEffect(() => {
  dispatch(goalsFetchAction(workspaceId?._id));
 }, [workspaceId?._id])
