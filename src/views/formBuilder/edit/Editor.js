...
export default function Editor({
  ...
  function checkEditElement(element) {
    ...
    if(gjsType) {
      let editTypeList = ['heading', 'paragraph', 'bullet', 'short-text', 'long-text', 'address', 'mym_text_box', 'signature', 'checkbox'];
  ...
  useEffect(() => {
    ...
    let styles = FontFamily.families.map((fontFam => {
      return placeHolders.map(placeHolder => {
        return `.${fontFam.split(' ').join('-')}::${placeHolder} { font-family: ${fontFam}; }`
      })
    })).reduce((prev, fonts) => prev.concat(fonts), []).join('\n')
    ...
    gjsEditor.Commands.add('set-data', (editor) => {
      const formData = store.form.formData.map((x) => {
        if (x.id === step.id) {
          return { ...x, html: editor.getHtml(), css: editor.getCss() , components:editor.getComponents() };
        }
        return x;
      });
      dispatch(
        ...
        for (const child of children) {
          if (child.classList.contains('add-new-section')) {
            ...
            child.addEventListener('click', (e) => {
              ...
              if (e.target !== e.currentTarget && !child.classList.contains('html-container')) return;
