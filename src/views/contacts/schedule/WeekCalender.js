...
const WeekCalender = (props) => {
  // ** Just Befor Mapping
  const { arrToMap, setArrToMap } = props;
  ...
  useEffect(() => {
    ...
    empShowArrToMap.map((employee, index1) => {
      ...
      employeePosition.map((position, index2) => {
        if (employee.position == position.position) {
          flag = 1;
        } else {
          return;
        }
  ...
  // ** Create and Update Shift Modal WeekDay style change
  const scheduleWeekdayClick = (e, index) => {
    e.target.closest('.schedule-weekday').classList.toggle('active');
    // if (
    //   e.target.closest('.schedule-weekday').classList.contains('active') &&
    //   shiftWeekDay.indexOf(index) == -1
    // ) {
    //   setShiftWeekDay([...shiftWeekDay, index]);
    // } else if (
    //   !e.target.closest('.schedule-weekday').classList.contains('active') &&
    //   shiftWeekDay.indexOf(index) > -1
    // ) {
    //   let removeIndex = shiftWeekDay.indexOf(index);
    //   shiftWeekDay.splice(removeIndex, 1);
    // } else return;
  };
            ...
            <thead>
              <tr>
                <th className="border cursor-pointer">
                  <div className="d-flex">
                    <div className="m-1">
                      <AddEmpolye empArrToMap={empArrToMap} setEmpArrToMap={setEmpArrToMap} />
                    </div>
                  </div>
                </th>
                {days.map((day, index) => (
                    ...
                    <div className="text-center d-flex justify-content-between align-items-center p-25">
                      ...
                      <div className="text-secondary ms-25 ">
                        <img
                          ...
                          height="40"
                        />
                        <span className="ms-50 font-small-2">
                          {weatherResult.list
                            ? celToFaConverter(weatherResult?.list[index]?.main?.temp)
                            : 'XX'}
                          &deg;
                        </span>
                        <div className="d-flex align-items-center justify-content-center font-small-2">
                          <User size={9} />
                      ...
                      return (
                        ...
                        >
                          ...
                          {empShowArrToMap !== undefined &&
                            empShowArrToMap
                              .filter((item) => item.position === positionItem.position)
                              .map((item, index) => {
                                return (
                                  <tr
                                    key={item.id}
