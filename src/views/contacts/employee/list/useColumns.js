...
export const useColumns = ({ setDeleteModal }, { toggle }, { setRow },{empStore}) => {
  // let count = [];
  ...
  const columns = [
    ...
    // },
    {
      ...
      <Badge
          ...
          pill
          color={empStore?.leadSources?.find((y) => y.title === row?.leadSource)?.color}
          className="me-50"
        >
          {row?.leadSource}
        </Badge>
      ...
      cell: (row) =>
        ...
        row?.tags?.map((x, i) => (
          <Badge
            key={i + 1}
            pill
            color={empStore?.tags?.find((y) => y.value === x)?.color}
            className="me-50"
          >
            {x}
          </Badge>
